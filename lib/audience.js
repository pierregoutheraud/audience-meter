...
Audience.prototype.namespace = function(name, username, auto_create)
{
    if (!name) return;
    ...
    {
      namespace.usernamesAdded.push(username);
    }
...
{
    var self = this,
        namespace = this.namespace(namespaceName, username);
...
Audience.prototype.notifyAll = function()
{
    for (var key in this.namespaces)
    {
        ...
        var usernames = namespace.usernamesAdded.slice(); // Copy of array
        namespace.usernamesAdded.length = 0 // Flush usernames queue
...
{
    var namespace = this.namespace(namespaceName, null, false);
