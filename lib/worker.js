...
function Worker(options)
{
    ...
    process.on('message', function(msg)
    {
        ...
        {
            case 'notify':
                self.notify(msg.namespace, msg.usernames, msg.msg);
                break;
    ...
    server.on('request', function(req, res)
    {
        ...
        else if ((pathInfo = pathname.match(path2ns)))
        {
            ...
            if (res.socket && req.headers.accept && req.headers.accept.indexOf('text/event-stream') != -1)
            {
                ...
                self.join(namespace, res, uuid, params);
...
Worker.prototype.join = function(namespace, client, uuid, params)
{

    var self = this;
    // Subscribe
    var user = this.groups.get(namespace, true, params).addClient({client:client, params:params});
