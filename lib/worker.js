...
{
    var _this = this;
    var user = this.lives.get(namespace, true).addUser({
      res:res,
      params:params
    });

    // Increment namespace counter on master (after a configured delay to mitigate +/- flood)
    user.res.incrementTimeout = setTimeout(function()
    {
      // JOIN :O
      // process.send({cmd: 'join', namespace: namespace, username:user.username});
      delete user.res.incrementTimeout;
    }, this.options.increment_delay * 1000);

    // Decrement on exit
    user.res.socket.on('close', function()
    {
        // Notify about the decrement only if the increment happended
        if ('incrementTimeout' in user.res)
        {
            clearTimeout(user.res.incrementTimeout);
        }
        else
        {
          // LEAVE ! :O
            // process.send({cmd: 'leave', namespace: namespace, username:user.username});
        }
